...
if __name__ == '__main__':
    ...
    with tab1:
        ...
        st.metric(f"{latest} 총 리뷰 수", f"{int(latest_sum)}", f"{int(latest_sum - previous_sum)}")
        ...
        col5 = st.columns(1)[0]
        col5.metric("스피드랙MAX", f"{int(latest_list[4])}" +" /" + f"{ratios[4]: .1%}", f"{ratios_delta[4]: .2%}" + "_" + f"{int(subtracts[4])}")
